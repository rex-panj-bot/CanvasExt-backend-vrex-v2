...
class DocumentManager:
    ...
    def add_files_to_catalog(self, file_paths: List[str]):
        ...
        for file_path in file_paths:
            ...
            # CRITICAL: Match _build_catalog behavior for consistency
            # - GCS mode: Keep full filename with extension in doc_id
            # - Local mode: Strip extension from doc_id
            if '/' in file_path:
                # GCS mode - keep extension (matches _build_catalog GCS logic)
                original_name = filename
                doc_id = f"{course_id}_{filename}"
            else:
                # Local mode - strip extension (matches _build_catalog local logic)
